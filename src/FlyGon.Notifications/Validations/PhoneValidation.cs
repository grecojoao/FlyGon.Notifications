...
{
    internal class PhoneValidation
    {
        private readonly PhoneValidationOptions _validationOptions;
        private static readonly Regex _defaultFormat =
            new Regex(@"^(?:(?:\+?55)?[ .-]*\(?0?[1-9][0-9]\)?)[ .-]?(?:(?:(?:9[ .-]*)?[0-9]{4})[ .-]*[0-9]{4})$");

        public PhoneValidation(PhoneValidationOptions validationOptions) =>
